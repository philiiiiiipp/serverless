          ...
          f()
            ...
            .then(resolve, e => {
              if (
                ...
                const exponentialBackOffTime = Math.round(
                  5000 + nextTryNum * Math.random() * 10000
                );

                that.serverless.cli.log(
                  _.join(
                    [
                      `Recoverable error occurred (${e.message}), sleeping for ${exponentialBackOffTime} seconds.`,
                      `Try ${nextTryNum} of ${MAX_TRIES}`
                    ],
                    " "
                  )
                );
                setTimeout(doCall, exponentialBackOffTime, nextTryNum);
              } else {
                reject(e);
              }
            });
